...
public class JobController {
	...
	private void runJob(int jobNumber) {
		...
		Set<UrlEntity> entitiesToDelete = new TreeSet<UrlEntity>();
					...
					if (!tweetWebCollection.isEmpty()) {
						for (WebCollection wc: tweetWebCollection) {
							if (expandStatus) {
								...
								ue.setCollectionName(wc.getName());
								ue.setTotalRetweets(tweet.getRetweetCount());
								//urlEntityDao.persist(new UrlEntity(urlInProgress,expandedUrl,u.getHost(),tweet.getId(),wc.getId(),wc.getName(),(int) tweet.getRetweetCount()));
								
							} else {
								//tweet.getUrlEntities().remove(ue);
								urlEntityFailedDao.persist(new UrlEntityFailed(urlInProgress,tweet.getId(),"Expand Failed",wc.getId(),wc.getName()));
								entitiesToDelete.add(ue);
							}
						...
						if (expandStatus) {
							ue.setUrlFull(expandedUrl);
							ue.setUrlDomain(u.getHost());
							ue.setWebCollection(null);
							ue.setCollectionName("");
							ue.setTotalRetweets(tweet.getRetweetCount());
							//urlEntityDao.persist(new UrlEntity(urlInProgress,expandedUrl,u.getHost(),tweet.getId(),(long) 0,"",(int) tweet.getRetweetCount()));
						} else {
							//tweet.getUrlEntities().remove(ue);
							urlEntityFailedDao.persist(new UrlEntityFailed(urlInProgress,tweet.getId(),"Expand Failed",(long) 0,""));
							entitiesToDelete.add(ue);
						}
					}
		...
		// delete the entities
		for (UrlEntity urlEntity : entitiesToDelete) {
			urlEntity.getTweet().getUrlEntities().remove(urlEntity);
			tweetDao.persist(urlEntity.getTweet());
	...
	private boolean expandUrl() {
		...
		try {
			...
			try {
				...
				log.error("JobController.getExpandedUrl : Error connecting to url : " + e.getMessage());
				result = false;
			} finally {
				con.disconnect();
				con = null;
			...
			log.error("JobController.getExpandedUrl : Error creating url from string : " + e.getMessage());
			result = false;
		}
